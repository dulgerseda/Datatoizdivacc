...
def get_model():
    model = joblib.load("project/model_train_all2.joblib")
    return model


model = get_model()
dforig = pd.read_excel("project/veriler_tum.xlsx")

tab_home, tab_oneri = st.tabs(["Hoşgeldiniz", "Aşk Testi"])

a1, a2, a3, a4 = tab_home.columns(4)
a2.image("project/seda.PNG", width=600)
...
def main():
    ...
    audio_file = open("project/music.mp3", "rb").read()
